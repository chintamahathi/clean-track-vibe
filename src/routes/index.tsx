    ...
    <div className="px-5 pt-6">
      <header className="flex items-center justify-between">
        <div className="flex min-w-0 items-center gap-3">
          <span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-[linear-gradient(135deg,var(--emerald),var(--cyan))] text-sm font-extrabold text-primary-foreground shadow-lift">A</span>
          <div className="min-w-0">
            ...
            <div className="mt-0.5 flex items-center gap-1 text-[11px] font-semibold text-muted-foreground">
              <MapPin className="size-3 text-emerald" />{resident.area}
            </div>
          </div>
        </div>
        <ProfileMenu />
      ...
      <section className="animate-float-in relative mt-5 overflow-hidden rounded-[2rem] bg-forest p-6 text-ivory shadow-float">
        ...
        <div className="relative">
          ...
          <div className="mt-3 flex items-center gap-2">
            <span className="rounded-full bg-ivory/10 px-3 py-1 text-[11px] font-bold tracking-wider text-ivory/85 ring-1 ring-ivory/15">{truck.id}</span>
      ...
      <section className="animate-sheet-up glass-panel mt-4 flex items-start gap-3 rounded-3xl p-4" style={{ animationDelay: "120ms" }}>
        <span className="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-soft text-emerald"><Truck className="size-5" /></span>
        <div className="min-w-0">
          <p className="text-[10px] font-extrabold tracking-[0.16em] text-emerald">TRUCK NEARBY</p>
          <p className="mt-0.5 text-sm font-bold text-forest">{truck.id} is approximately 3 minutes away.</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        <Link to="/requests" className="group rounded-3xl bg-card p-4 shadow-card transition-transform hover:scale-[1.02]">
          ...
          <span className="mt-3 flex size-8 items-center justify-center rounded-full bg-secondary text-forest transition-transform group-hover:translate-x-1"><ChevronRight className="size-4" /></span>
        </Link>
        <Link to="/bills" className="group rounded-3xl bg-pale p-4 shadow-card transition-transform hover:scale-[1.02]">
          <span className="flex size-8 items-center justify-center rounded-full bg-emerald-soft text-emerald text-base">💳</span>
          <p className="mt-3 text-sm font-extrabold text-forest">Bills</p>
          <p className="text-[11px] font-semibold text-muted-foreground">₹0 · No pending payments</p>
        </Link>
      </div>
      ...
      <Link to="/points" className="group mt-4 flex items-center gap-3 rounded-3xl bg-card p-4 shadow-card transition-transform hover:scale-[1.01]">
        <span className="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-amber-soft text-[oklch(0.6_0.13_70)]"><MapPin className="size-5" /></span>
    ...
    <section className="animate-float-in mt-4 rounded-[2rem] bg-card p-5 shadow-card" style={{ animationDelay: "240ms" }}>
      <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center gap-3 text-left">
        ...
        <span className="min-w-0 flex-1">
          ...
          <span className="block text-[11px] text-muted-foreground">{collectionSchedule.frequency} · {collectionSchedule.window} · {collectionSchedule.vehicle}</span>
        </span>
