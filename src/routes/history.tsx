...
function History() {
  ...
  return (
    <div className="px-5 pt-6">
      <SubHeader title="History" subtitle="Your last 7 days of collections." />
      ...
      <ul className="mt-4 space-y-3 pb-4">
        {history.map((h, i) => (
          <li key={h.id} className="animate-float-in flex items-center gap-4 rounded-3xl bg-card p-4 shadow-card" style={{ animationDelay: `${i * 60}ms` }}>
            <div className="flex size-12 shrink-0 flex-col items-center justify-center rounded-2xl bg-pale">
              ...
              <span className="text-[9px] font-bold uppercase tracking-wide text-muted-foreground">{h.date.split(" ")[0]}</span>
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-extrabold text-forest">{h.window}</p>
              <p className="text-[11px] text-muted-foreground">
                {h.vehicle !== "—" ? h.vehicle : "No vehicle dispatched"}
                {h.weightKg > 0 && <span className="ml-2 inline-flex items-center gap-1 font-semibold text-emerald"><Weight className="size-3" /> {h.weightKg} kg</span>}
              </p>
            </div>
